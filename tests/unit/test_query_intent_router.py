...
from src.intent_detection.types import IntentDetectionConfig, IntentType, QueryIntent
...
class TestKeywordIntentDetector:
    ...
    def test_special_characters(self):
        ...
        for query in queries:
            ...
            assert result.intent_type in [
                IntentType.FACT_QA,
                IntentType.RELATIONSHIP_DISCOVERY,
            ]
    ...
    def test_confidence_calculation(self):
        ...
        )  # Should have good confidence with strong keyword
...
class TestLLMIntentDetector:
    ...
    def test_llm_classification_relationship(self, mock_llm_adapter):
        ...
        mock_llm_adapter.classify_intent.assert_called_once_with(
            "How are these tech companies connected?"
        )
    ...
    def test_llm_error_handling(self, mock_llm_adapter):
        ...
        mock_llm_adapter.classify_intent.side_effect = Exception(
...
class TestIntentRouter:
    ...
    def router(self):
        ...
        config = IntentDetectionConfig(
            keyword_threshold=0.6, llm_threshold=0.7, use_llm_fallback=True
        )
    ...
    def test_logging_functionality(self, router):
        ...
        with patch("src.intent_detection.router.logger") as mock_logger:
            ...
            with patch.object(
                router, "_route_query_cached", side_effect=Exception("Test error")
            ):
...
class TestEdgeCases:
    ...
    def test_very_long_queries(self, router):
        ...
        for target_length in [100, 500, 1000, 5000, 10000]:
            ...
            test_cases.append(
                (target_length, query[: target_length + 100])
            )  # Allow some extra
        ...
        for length, query in test_cases:
            ...
            result = router.route_query(query)
            assert (
                result["intent_type"] == "relationship_discovery"
            )  # Should detect "similar"
