...
from src.intent_detection.types import IntentDetectionConfig, IntentType
...
class MockHybridRetriever:
    ...
    async def retrieve(
        self, query: str, relationship_types: list = None
    ) -> dict[str, Any]:
...
class TestQueryIntentRouting:
    ...
    @pytest.mark.asyncio
    async def test_relationship_routing_to_hybrid_retriever(
        self, router, hybrid_retriever
    ):
    ...
    @pytest.mark.asyncio
    async def test_multiple_queries_routing(
        self, router, vector_retriever, hybrid_retriever
    ):
    ...
    @pytest.mark.parametrize(
        "error_scenario",
        ["retriever_timeout", "retriever_error", "invalid_intent_type"],
    )
    def test_error_handling_with_downstream(self, router, error_scenario):
        ...
        if error_scenario == "retriever_timeout":
            ...
            failing_retriever.retrieve.side_effect = Exception(
    ...
    async def test_full_pipeline_fact_qa(self):
        ...
        if routing_result["intent_type"] == IntentType.FACT_QA.value:
            retrieval_result = await vector_retriever.retrieve(
                query, top_k=routing_result.get("suggested_top_k", 5)
            )
