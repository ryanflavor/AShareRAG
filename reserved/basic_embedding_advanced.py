...
class MemoryInfo:
    ...
    def __str__(self):
        return (f"GPU: {self.gpu_used:.2f}/{self.gpu_total:.2f}GB "
...
class PerformanceMetrics:
    ...
    def __str__(self):
        return (f"处理 {self.texts_count} 个文本，耗时 {self.processing_time:.2f}秒\n"
...
def auto_retry_on_oom(max_retries: int = 3, 
                     wait_time: float = 2.0,
...
class Qwen3EmbeddingManager:
    ...
    def __init__(self, 
        ...
        self.embedding_dim = embedding_dim or int(os.getenv("QWEN3_EMBEDDING_DIM", "2560"))
        self.model = None
    ...
    def search(self, 
        ...
        # 编码文档
        doc_embeddings = self.encode_texts(documents, show_progress=len(documents) > 100)
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'peak_gpu_memory': max(m.memory_after.gpu_used for m in self.performance_history)
        }
...
def demo_semantic_search():
    ...
    for query in queries:
        ...
        for result in results:
            print(f"{result['rank']}. [{result['similarity']:.4f}] {result['document']}")
...
def demo_batch_performance():
    ...
    for text_count in test_sizes:
        test_texts = [f"测试文本 {i}: 这是用于性能测试的示例文本内容" for i in range(text_count)]
        ...
        for batch_size in batch_sizes:
            with memory_management(f"Batch-{batch_size}"):
                ...
                if embeddings is not None:
                    ...
                    print(f"  批大小 {batch_size:2d}: {latest_metric.throughput:6.1f} texts/秒")
...
def demo_dimension_comparison():
    ...
    if results:
        ...
        print("-" * 35)
        for r in results:
            print(f"{r['dimension']:>6} | {r['throughput']:>10.1f}/s | {r['memory_used']:>8.2f}GB")
