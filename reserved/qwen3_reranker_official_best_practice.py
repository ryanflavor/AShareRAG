...
class Qwen3RerankerOfficial:
    ...
    def _log_memory_usage(self):
        ...
        if torch.cuda.is_available():
            ...
            self.logger.debug(f"GPU 内存: 已分配 {allocated:.2f}GB / 已保留 {reserved:.2f}GB")
    ...
    def rerank(self,
               ...
               return_metrics: bool = False) -> Tuple[List[RerankResult], Optional[PerformanceMetrics]]:
        """
        重排序文档
            ...
            with self._memory_efficient_mode():
                for i in range(0, len(documents), batch_size):
                    ...
                    try:
                        ...
                        self.logger.debug(f"处理批次 {i//batch_size + 1}/{(len(documents)-1)//batch_size + 1}")
...
def main():
    ...
    if metrics:
        ...
        print(f"- 分数范围: {metrics.score_range[0]:.4f} - {metrics.score_range[1]:.4f}")
    ...
    for doc in ranked_docs:
        print(f"ID: {doc['id']}, 分数: {doc['rerank_score']:.4f}, 来源: {doc['source']}")
        print(f"内容: {doc['text']}")
        print()
