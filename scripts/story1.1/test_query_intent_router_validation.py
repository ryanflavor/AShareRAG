...
def run_validation_test():
    ...
    router_keyword = IntentRouter(
        config=IntentDetectionConfig(use_llm_fallback=False, cache_enabled=False)
    )
    ...
    for test in VALIDATION_TESTS:
        if test["category"] in ["keyword_detection", "default_fact_qa", "edge_case"]:
            ...
            print(
                f"{status} {test['id']:8} | {latency:6.2f}ms | {result['intent_type']:20} | {test['query'][:30]}"
            )
    ...
    print(
        "  ✓ Meets AC11 requirement (< 100ms)"
        if max_keyword_latency < 100
        else "✗ Fails AC11 requirement"
    )
    ...
    if llm_latencies:
        ...
        print(
            "  ✓ Within acceptable range (15-20s)"
            if avg_llm_latency < 20000
            else "⚠️  Exceeds 20s but still acceptable"
        )
    ...
    router_cache = IntentRouter(
        config=IntentDetectionConfig(
            use_llm_fallback=True, cache_enabled=True, keyword_threshold=0.8
        )
    )
    ...
    print(
        f"  First call (miss): {latency1:.1f}ms, cache_hit={result1.get('cache_hit', False)}"
    )
    ...
    ac_results = [
        (
            "AC2",
            "Chinese keywords trigger relationship_discovery",
            all(
                r["correct"]
                for r in results["test_results"]
                if r["category"] == "keyword_detection"
            ),
        ),
        (
            "AC3",
            "Default to fact_qa when no keywords",
            all(
                r["correct"]
                for r in results["test_results"]
                if r["category"] == "default_fact_qa"
            ),
        ),
        (
            "AC10",
        ...
        (
            ...
            .get("llm_classification", {})
            .get("meets_requirement", True),
        ),
        (
            "AC11.3",
    ...
    for ac_id, description, passed in ac_results:
        ...
        results["acceptance_criteria"].append(
            {"id": ac_id, "description": description, "passed": passed}
        )
    ...
    output_file = (
        output_dir
        / f"story_2.1_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
